  ...
  this.choices = function (nDeep) {
    ...
    firstChoices.forEach(function (choice) {
      if (choice === dg.epsilon()) {
        nChoices[choice] = null
      } else {
        guide.choose(choice)
        var nextChoices = guide.choices(nDeep - 1)
        if (Array.isArray(nextChoices)) {
          // nDeep = 1, so this array needs to be transformed into an object
          var lastChoices = {}
          nextChoices.forEach(function (nextChoice) {
            lastChoices[nextChoice] = null
          })
          nChoices[choice] = lastChoices
        } else {
          nChoices[choice] = nextChoices
        }
        guide.pop()
      }
    })
 ...
 * @see {@link GuidedDecisionGraph#choices}
 */
